...
- 可选模型网关隐藏 `/v1/models` 中的别名，同时继续转发别名调用；日志可下载仅用户/助手文本的加密对话记录。
...
默认情况下，模型调用仍直接使用 `http://你的域名:8317/v1`。需要隐藏模型别名并下载对话记录时，启用下面的可选网关。
...
经网关处理的 Responses、Chat Completions、Messages 和 Gemini 对话请求，会仅提取用户与助手的文本并加密保存；系统/开发者指令、工具调用及结果、图片和其他原始协议事件不保留。同一用户反复提交的相同历史消息只保存一次，不同用户之间不共用；下载仍按每次请求重建当次对话，新版也能读取旧版记录。用户可下载自己的记录，管理员可下载全局记录。每位用户最多保留最近 100 条、最长 7 天；请求或响应原始数据单侧超过 16 MB 时标注“已截断”，调用仍正常转发。请求头中的 API Key、Authorization 和门户 Cookie 不会写入记录；用户自行写进对话的秘密仍属于对话文本。未经网关的旧请求或直连 CPA 的请求没有可下载对话。更换 `PORTAL_APP_SECRET_FILE` 会使已有加密记录无法解密。
...
| `PORTAL_GATEWAY_CAPTURE_DIR` | `/data/gateway-captures` | 加密对话记录目录 |
