...
func TestMergeRefreshedQuotaPrefersNewerPersistedValueAndResetTogether(t *testing.T) {
	now := time.Date(2026, 9, 7, 12, 30, 0, 0, time.UTC)
	oldObserved := now.Add(-time.Minute).UnixMilli()
	newObserved := now.UnixMilli()
	oldReset := now.Add(54 * time.Minute).UnixMilli()
	newReset := now.Add(3*time.Hour + 58*time.Minute).UnixMilli()
	oldRemaining, newRemaining := 98.0, 99.0
	keys := NewKeys(nil, nil)
	keys.refreshedQuota["two.json\x00two"] = []cpamp.QuotaSnapshotWindow{{
		WindowKind: "five_hour", ModelScopeKind: "all", ObservedAtMS: oldObserved,
		CycleEndMS: &oldReset, RemainingPercent: &oldRemaining, PlanType: "plus", Availability: "active",
	}}
	items := []cpamp.QuotaSnapshotItem{{RowKey: "two.json\x00two", Provider: "codex", Windows: []cpamp.QuotaSnapshotWindow{{
		WindowKind: "five_hour", ModelScopeKind: "all", ObservedAtMS: newObserved,
		CycleEndMS: &newReset, RemainingPercent: &newRemaining, PlanType: "plus", Availability: "active",
	}}}}

	merged := keys.mergeRefreshedQuota(items, now)
	selected := currentQuotaWindows(merged[0].Windows, now)
	if len(selected) != 1 || quotaRemaining(selected[0].Window) != 99 {
		t.Fatalf("selected quota = %#v", selected)
	}
	if got := quotaWindowEffectiveReset("five_hour", selected[0].Window, selected, now); got.UnixMilli() != newReset {
		t.Fatalf("reset = %v, want %v", got, time.UnixMilli(newReset))
	}
	if len(keys.refreshedQuota) != 0 {
		t.Fatalf("superseded refresh cache was retained: %#v", keys.refreshedQuota)
	}
}

func TestMergeRefreshedQuotaKeepsNewerManualValueAndResetTogether(t *testing.T) {
	now := time.Date(2026, 9, 7, 12, 30, 0, 0, time.UTC)
	oldObserved := now.Add(-time.Minute).UnixMilli()
	newObserved := now.UnixMilli()
	oldReset := now.Add(54 * time.Minute).UnixMilli()
...
func TestUpstreamQuotaReadsAutomaticUpdatesWithoutAStalePoolCache(t *testing.T) {
	now := time.Date(2026, 9, 7, 12, 30, 0, 0, time.UTC)
	queries := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v0/management/auth-files":
			_ = json.NewEncoder(w).Encode(map[string]any{"files": []map[string]any{{"name": "one.json", "provider": "codex", "auth_index": "one"}}})
		case "/v0/management/quota-snapshots/query":
			queries++
			remaining := 98.0
			...
			_ = json.NewEncoder(w).Encode(cpamp.QuotaSnapshotQueryResponse{Items: []cpamp.QuotaSnapshotItem{{
				RowKey: "one.json\x00one", Provider: "codex", Windows: []cpamp.QuotaSnapshotWindow{{
					WindowKind: "five_hour", ModelScopeKind: "all", ObservedAtMS: now.Add(time.Duration(queries) * time.Minute).UnixMilli(),
					CycleEndMS: &reset, RemainingPercent: &remaining, PlanType: "plus", Availability: "active",
				}},
			}}})
		default:
			http.NotFound(w, r)
	...
	keys := NewKeys(nil, client)
	keys.Now = func() time.Time { return now }
	for wantCall, wantRemaining := range []int{98, 99} {
		pool, err := keys.UpstreamQuota(context.Background())
		if err != nil {
			t.Fatal(err)
		}
		if len(pool.Groups) != 1 || pool.Groups[0].RemainingPercent != wantRemaining {
			t.Fatalf("call %d pool = %#v", wantCall+1, pool)
		}
	}
	if queries != 2 {
